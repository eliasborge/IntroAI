...
class CSP:
    ...
    def backtrack(self, assignment):
        ...
        ### Returns the assigment if all assigments only are 1 in length.
        ...
        for val in assignment[var]:
            ...
            ## Starts recursive backtrack if inference is found.
            if self.inference(assignmentCopy,self.get_all_neighboring_arcs(var)):
                res = self.backtrack(assignmentCopy)
                if res!= None:
                    return res
        return None
    ...
    def select_unassigned_variable(self, assignment):
        ...
        """
        # TODO: YOUR CODE HERE
        ## Sorts variables and returns the variable with the least remaining values.
    ...
    def inference(self, assignment, queue):
        ...
        """
        # TODO: YOUR CODE HERE
        ## Method for reducing the possible values of each variable. Uses the constraints to do this.

        while len(queue) != 0:
            #Visit arc
            (i,j) = queue.pop()
            #Revise to check that it is correct
            if self.revise(assignment,i,j):
                if(len(assignment[i]) == 0):
                    return False 
                
                ##Checks if the arc is a neighbour.
                neighbours = self.get_all_neighboring_arcs(i)
                if((i,j) in neighbours):
                    neighbours.remove((i,j))
                if((j,i) in neighbours):
                    neighbours.remove((j,i))
    ...
    def revise(self, assignment, i, j):
        ...
        ##access all the i-values
        for val_i in assignment[i]:
            #Default set to false for the boolean value that controls if the constraints are satisfied.
            satisfies = False
            ##Iterate over all the j-values in relation to the i-values.
            for val_j in assignment[j]:
                if((val_i,val_j) in self.constraints[i][j]):
                    satisfies = True

            #Remove from the value from the assigments if it doesnt satisfy the constraints.
            if not satisfies:
                removeArray.append(val_i)
                rev = True
        for val in removeArray:
           # print(assignment[i])
...
csp1 = create_sudoku_csp("Assignment4/easy.txt")
csp2 = create_sudoku_csp("Assignment4/medium.txt")
csp3 = create_sudoku_csp("Assignment4/hard.txt")
csp4 = create_sudoku_csp("Assignment4/veryhard.txt")
...
print("Hard")
sol3 = csp3.backtracking_search()
print_sudoku_solution(sol3)

sol4 = csp4.backtracking_search()
print("Very Hard")
